    ...
    for dir_or_file in os.listdir(source_path):
        ...
        if os.path.isfile(file_path):  # 判断是否为文件
            file_name_ext = os.path.splitext(
                os.path.basename(file_path))  # 文件名与后缀
...
class CustomDataset(Dataset):
    def __init__(self, file_path, transform=None):
        ...
        """
        env_path_list = []
        find_image_file(file_path, env_path_list)
        label = [x.split('/')[-2] for x in env_path_list]
        rawdata = pd.DataFrame([env_path_list, label],
                               index=['path', 'label']).T
        class_labels = rawdata.label.value_counts().index.tolist()
        self.label = rawdata.label.apply(lambda x: class_labels.index(x)).values
        self.data_frame = rawdata
    ...
    def __init__(
        ...
        self.transforms = transforms.Compose(
            [transforms.ToTensor(), transforms.Normalize(
                (0.1307,), (0.3081,)), transforms.Resize((256, 256))]
        )
    ...
    def setup(self, stage: Optional[str] = None) -> None:
        ...
        if not self.data_train and not self.data_val and not self.data_test:
            dataset = CustomDataset(
                file_path=self.hparams.data_dir, transform=self.transforms)
